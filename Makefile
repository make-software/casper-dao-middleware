...
sync-db:
	sh -ac '. apps/handler/.env; migrate -database "mysql://$$DATABASE_URI" -path internal/resources/migrations up'

sync-test-db:
	sh -ac '. internal/tests/.env.test; migrate -database "mysql://$$TEST_DATABASE_URI" -path internal/resources/migrations up'
