...
func (s *TrackContract) Execute() error {
	...
	case simple_voter.VotingCreatedEventName:
		...
		if err := trackVotingCreated.Execute(); err != nil {
			zap.S().With(zap.String("event", cesEvent.Name)).
				With(zap.String("contract", doaContractMetadata.SimpleVoterContractHash.String())).Info("failed to track event")
			return err
		}
	case simple_voter.VotingEndedEventName:
	...
	case simple_voter.VotingCanceledEventName:
	...
	case simple_voter.BallotCanceledEventName:
