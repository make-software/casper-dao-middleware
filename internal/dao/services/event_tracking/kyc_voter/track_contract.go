...
func (s *TrackContract) Execute() error {
	...
	case kyc_voter.VotingCreatedEventName:
		...
		if err := trackVotingCreated.Execute(); err != nil {
			zap.S().With(zap.String("event", cesEvent.Name)).
				With(zap.String("contract", doaContractMetadata.KycVoterContractHash.String())).Info("failed to track event")
			return err
		}
	case kyc_voter.VotingEndedEventName:
	...
	case kyc_voter.VotingCanceledEventName:
	...
	case kyc_voter.BallotCanceledEventName:
