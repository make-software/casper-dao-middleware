...
import (
	...
	"casper-dao-middleware/internal/services/event_tracking"
...
type TrackBallotCastTestSuit struct {
	suite.Suite
...
func (suite *TrackBallotCastTestSuit) SetupTest() {
...
func (suite *TrackBallotCastTestSuit) TestTrackBallotCast() {
	deployResult := casper.GetDeployResult{}
	err := json.Unmarshal([]byte(createBallotCastDeployJSON), &deployResult)
	assert.NoError(suite.T(), err)
	...
	for _, event := range daoEvents {
		if event.EventName == events.BallotCastName {
			trackBallotCast.SetEventBody(event.EventBody)
		}
	}

	err = trackBallotCast.Execute()
	assert.NoError(suite.T(), err)

	votes, err := suite.entityManager.VoteRepository().Find(&pagination.Params{
		OrderDirection: pagination.OrderDirectionDESC,
		Page:           1,
		PageSize:       10,
	}, nil)
	assert.NoError(suite.T(), err)
	assert.True(suite.T(), len(votes) == 1)

	assert.Equal(suite.T(), votes[0].Amount, uint64(1000))
	assert.Equal(suite.T(), votes[0].IsInFavor, true)
	assert.Equal(suite.T(), votes[0].VotingID, uint32(1))
}

func TestTrackBallotCastTestSuit(t *testing.T) {
	suite.Run(t, new(TrackBallotCastTestSuit))
}

var createBallotCastDeployJSON = `
{
  "deploy": {
